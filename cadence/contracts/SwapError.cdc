...
access(all) contract SwapError {
    access(all) enum ErrorCode: UInt8 {
        ...
        access(all) case FLASHLOAN_EXECUTOR_SETUP
        access(all) case FEE_TO_SETUP
        access(all) case BELOW_MINIMUM_INITIAL_LIQUIDITY // 15
    }

    access(all) view fun ErrorEncode(msg: String, err: ErrorCode): String {
